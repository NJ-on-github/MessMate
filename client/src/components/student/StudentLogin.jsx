...
const StudentLogin = () => {
  ...
  const [error, setError] = useState({ message: '', subMessage: '', showForm: true, isSuccess: false });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/student/login`, {
        method: 'POST',
        headers: {
  ...
  if (!error.showForm) {
    return (
      <div className="center-all">
        <div className="student-login-error-container">
          <h2>{error.message}</h2>
    ...
    <div className="page-background">
      <div className="center-all">
        <div className="form-container">
          ...
          {error.message && (
            <p className={`login-message ${error.isSuccess ? 'success' : 'error'}`}>
              {error.message}
            </p>
          )}
