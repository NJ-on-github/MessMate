...
//   try {
//     const adminResult = await pool.query(queries.MATCH_ADMIN_LOGIN, [email]);

//     // ADD THESE TWO LINES
//     console.log("Admin login attempt →", email);
//     console.log("Found rows:", adminResult.rows.length, adminResult.rows);
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    console.log("Found rows:", adminResult.rows.length);
...
router.get("/payments/summary", async (req, res) => {
  ...
  if (!month) {
    return res
      .status(400)
      .json({ error: "Month parameter is required (format: MM-YYYY)" });
  }
...
router.patch("/reject-registration/:id", async (req, res) => {
  try {
    await pool.query(queries.REJECT_REGISTRATION, [req.params.id]);
...
router.patch("/unblock-registration/:id", async (req, res) => {
  try {
    ...
    res.json({
      message: "Registration status updated to pending successfully.",
    });
  } catch (err) {
...
router.post("/insert-fee", async (req, res) => {
  ...
  try {
    ...
    if (new Date(effective_from) >= new Date(new Date().setDate(1))) {
      await client.query(queries.CREATE_PAYMENTS_FOR_NEW_FEE, [
        newFee.fee_id,
        newFee.monthly_fee,
        newFee.effective_from,
      ]);
    }
...
router.post("/menu/save-todays-menu", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "At least one menu item must be selected." });
    }
    ...
    if (checkMenu.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "Today's menu has already been saved." });
    }
    ...
    for (const item of breakfast) {
      await client.query(queries.INSERT_TODAYS_BREAKFAST, [
        todaysMenuId,
        item.id,
      ]);
    }
...
router.get("/menu/todays", async (req, res) => {
  try {
    ...
    if (menuRes.rows.length === 0)
      return res.status(404).json({ error: "Today's menu not set" });
    const menuId = menuRes.rows[0].id;

    // Get items for each meal
    const [breakfastRes, lunchRes, dinnerRes] = await Promise.all([
      pool.query(
        `SELECT b.id, b.name FROM todays_breakfast tb JOIN breakfast_items b ON tb.breakfast_item_id = b.id WHERE tb.todays_menu_id = $1`,
        [menuId]
      ),
      pool.query(
        `SELECT l.id, l.name FROM todays_lunch tl JOIN lunch_items l ON tl.lunch_item_id = l.id WHERE tl.todays_menu_id = $1`,
        [menuId]
      ),
      pool.query(
        `SELECT d.id, d.name FROM todays_dinner td JOIN dinner_items d ON td.dinner_item_id = d.id WHERE td.todays_menu_id = $1`,
        [menuId]
      ),
    ]);
